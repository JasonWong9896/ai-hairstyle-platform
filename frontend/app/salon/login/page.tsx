...
export default function SalonLoginPage() {
  ...
  const labels = copy[language].salonLogin
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (isRegister) {
        await register({ email, password, name, role: 'salon' })
        setPassword('')
        setMessage('Salon 确认邮件已发送。若这是未确认账号的重复注册，我们已重新发送确认链接。请查看邮箱后再登录。')
        setMode('login')
        return
      }

      const auth = await login({ email, password, role: 'salon' })
      saveAuth(auth)
      const redirect = new URLSearchParams(window.location.search).get('redirect')
      router.push(redirect ?? '/salon')
    } catch (err: any) {
      setError(err?.response?.data?.message ?? labels.error)
    } finally {
      setLoading(false)
    }
  }
          ...
          >
            ...
            {!isRegister && (
              <div className="mb-5 flex justify-end">
                <Link
                  href="/reset-password?role=salon"
            ...
            >
              ...
              {isRegister ? labels.submitRegister : labels.submitLogin}
            </button>
          </form>
        </section>
      </div>
